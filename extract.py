...
def basic_stats(x, row, prefix):
    ...
    if N > 2:
        ...
        row[prefix + "_mad"] = np.sum(np.abs(x -
                                             row[prefix + "_mean"])) / float(len(x))
...
def worker(indices):
    ...
    question1 = np.frombuffer(shared_question1, dtype=np.int32).reshape(
        (num_instances, max_sequence_length))
    question2 = np.frombuffer(shared_question2, dtype=np.int32).reshape(
        (num_instances, max_sequence_length))
    weights = np.frombuffer(shared_weights, dtype=np.float32).reshape(
...
def add_features(dataset, weights, nprocs):
    ...
    df_features = []
    pool = multiprocessing.Pool(processes=nprocs, initializer=init_shared, initargs=(
        shared_question1, shared_question2, shared_weights))
